...
public class DoubleJumpCube : MonoBehaviour
{
    ...
    float recreateDuration = 30.0f;
    public GameObject DoubleJumpParticle;
    void Update() {
        transform.Rotate(new Vector3(Time.time * Random.Range(-1.0f, 1.0f),Time.time * Random.Range(-1.0f, 1.0f),Time.time * Random.Range(-1.0f, 1.0f)));
    }
